...
author: "Vincent Tu (A15484541), Luis Mancilla (A16027554), Abirami Sabbani (A15487509)"
date: "3/9/2022"
output:
  html_document:
    df_print: paged
...
Since the train and test datasets have a different amount of data, the scale is different but the ratios are about the same. We also noticed that for the log transformation feature shows a high variance for features 56 and 57, but it is not as noticeable when we standardized the feature.
...
The features: 5, 7, 8, 10, 11, 13, 14, 15, 16, 17, 18, 20, 21, 23, 24, 25, 27, 28, 37, 42, 43, 44, 45, 46, 48, 52, 53, 54 are statistically significant. We also have features 55, 56, 57 as NA in the summary function this is because these features are singularities, meaning that 
their respective columns are either all 0s or all 1s so we cannot get a p-value from it.
...
The linear svm accuracy for the log transformed train data is 94.20%.
...
Use either a single method with properly chosen tuning parameter or a combination of several methods to design a classifier with test error rate as small as possible. Describe your recommended method, and report its performance. 


The log transformation gaussian SVM classifier has the best test accuracy rate of 96.21% based on the table.


Now we fine tune it even more to achieve a smaller test error

Original Tune: Cost = 10, gamma = 0.01, accuracy = 96.21%
...
We tuned 20 times to make our test error rate smaller. In conclusion, we found the optimal parameters for the gaussian classifier on the log transformation data is 
roughly: cost = 9.565, or cost = 9.575, or cost = 9.56, gamma = 0.017, or gamma = 0.0175 for an accuracy of 96.61%, an improvement of 0.4% compare to the original tuning parameter.
